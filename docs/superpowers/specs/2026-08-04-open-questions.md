...
Three layers (PRD 05, PRD 06, team-assess) with no shared name. Candidates are in the pull request description.

**Default:** keep referring to them by PRD number until someone picks, so nothing gets renamed twice.

### 2. Is PRD 06 built, or does PRD 05 ship alone first?
...
### 3. Does the classroom case drive the design, or the team case?
...
### 4. What is the run-to-run variance of one scoring pass? — blocking
...
### 5. Can the scorer reproduce the ground-truth before-and-after table?
...
### 6. Are the four editorial signals in scope for automated detection?
...
### 7. Does prosody require audio? (carried from PRD 05)
...
### 8. How is "confidently wrong" measured? (carried from PRD 06)
...
### 9. Who can read a snapshot? — blocking
...
### 13. Which speech-to-text provider, and is diarization good enough?
...
### 15. Does team-assess run on demand or on a schedule?

The spec's CLI is manual; the trend feature implies a cadence.

**Default:** manual, because a scheduled job emitting noisy trend labels compounds question 4 into a recurring notification nobody trusts.
